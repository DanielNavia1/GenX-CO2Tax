...
function write_net_revenue(path::AbstractString, sep::AbstractString, inputs::Dict, setup::Dict, EP::Model, dfCap::DataFrame, dfESRRev::DataFrame, dfResRevenue::DataFrame, dfChargingcost::DataFrame, dfPower::DataFrame, dfEnergyRevenue::DataFrame, dfSubRevenue::DataFrame, dfRegSubRevenue::DataFrame)
	...
	elseif v"1.4" <= VERSION < v"1.7"
 	...
 	if setup["CO2Cap"] >=1 && has_duals(EP) == 1 && setup["CO2Cap"]!=4
 		for y in 1:G
	...
	# NEW_CO2_TAX: Add CO2 Tax to the dataframe if CO2cap==4
	
	if setup["CO2Cap"] ==4

		if v"1.3" <= VERSION < v"1.4"
			dfNetRevenue[!,:CO2_Tax_cost] .= 0.0
		elseif v"1.4" <= VERSION < v"1.7"
			dfNetRevenue.CO2_Tax_cost = zeros(size(dfNetRevenue, 1))
		end
		for i in 1:G
			dfNetRevenue.CO2_Tax_cost[i] = sum(dfGen[i,:CO2_Tax_per_MWh] .* inputs["omega"] .* value.(EP[:vP])[i,:])
		end
		if setup["ParameterScale"] == 1
			dfNetRevenue.CO2_Tax_cost = dfNetRevenue.Fuel_cost * (ModelScalingFactor^2) # converting Million US$ to US$
		end
	end
