...
# Dharik - Done, we have scaled fuel costs above so any parameters on per MMBtu do not need to be scaled
	...
	inputs_gen["dfGen"][!,:CO2_Tax_per_MWh]=zeros(Float64,G)
	for g in 1:G
		...
		# NEW_CO2_TAX: Calculation of the costs associated with CO2 tax for each generator.
		# Note when "CO2cap"!=4, CO2_Tax_per_Ton is equal to 0 and the calculations here have no effect on the simulation.
		...
		if g in inputs_gen["COMMIT"]
			...
			# NEW: CO2 tax from fuel consumption during startup is also calculated and added to start-up cost
			...
			inputs_gen["C_Start"][g,:] = inputs_gen["dfGen"][!,:Cap_Size][g] * (fuel_costs[fuel_type[g]] .* start_fuel[g] .+ start_cost[g])+inputs_gen["CO2_Tax_per_Ton"]*inputs_gen["dfGen"][!,:CO2_per_Start][g]
