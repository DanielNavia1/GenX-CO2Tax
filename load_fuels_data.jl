...
function load_fuels_data(setup::Dict, path::AbstractString, sep::AbstractString, inputs_fuel::Dict)
	...
	# NEW_CO2_TAX: 
	# Read the CO2 tax from the A2 cell in Fuels_data.csv, if the corresponding CO2cap==4 is set in the settings file.
	# Otherwise, the CO2 tax is set to 0 in subsquent steps.

	if setup["CO2Cap"]==4
		CO2_Tax=fuels_in[1,1]  # Externally assumed price of CO2 in USD per ton (or equivalently carbon tax)
		inputs_fuel["CO2_Tax_per_Ton"]=CO2_Tax
	else
		inputs_fuel["CO2_Tax_per_Ton"]=0
	end
